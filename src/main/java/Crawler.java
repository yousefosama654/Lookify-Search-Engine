...
public class Crawler implements Runnable {
    ...
    public static void WriteSeeds() {
        try {
            FileWriter writer = new FileWriter("src/main/java/Seeds.txt",true);
            writer.write("Hello, world!");
            // for loop over the links u have got in the queue and write ib in the file after all the crawling work
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public Crawler(int n, int b) {
        ...
        if (fillFromDB()) {
            ...
            for (String seed : HTMLSeed) {
                ...
                Document doc1 = new Document("links", seed).append("normlizied", normalized(seed)).append("removed", "0");
